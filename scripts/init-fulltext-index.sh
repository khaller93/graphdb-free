...
if [ ! -z $1 -a $1 = "true" ]; then
	if [ ! -z $2 ] && [ ! -z $3 ]; then
		...
		iquery=$(./gen-graphdb-fts-config.sh $3)
		...
		do
			sleep $sleep_time
			echo "$iquery"
			status=$(curl -o /dev/null -w "%{http_code}" --request POST -H "Content-Type: application/sparql-update" http://localhost:7200/repositories/$2/statements --data "$iquery")
			case "$status" in
				2* ) 
					break
					;;
